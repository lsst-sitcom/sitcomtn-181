...
coordinates via the local WCS Jacobian at each guider position allows
...
\noindent
where $\mathbf{J}_i$ is the $2\times 2$ WCS Jacobian (the CD matrix
in FITS convention) evaluated at the position of guider $i$.  It
encodes:
\begin{enumerate}
...
\label{eq:mad}
\end{equation}
where the factor 1.4826 ensures consistency with the standard
deviation under Gaussian errors.
...
The covariance of the robust estimator is:
\begin{equation}
