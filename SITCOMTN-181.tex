...
\setDocAbstract{%
We present a framework for extracting the rotator oscillation angle
$\delta\theta(t)$ from focal-plane centroid measurements of guide
stars tracked by the four corner guider sensors of LSSTCam.  The
observed centroid displacements are modelled as a rigid-body
motion---common-mode translation $(dx, dy)$ and rotation
$\delta\theta$ about the optical axis---plus per-guider residuals from
atmospheric and optical distortions.  We fit the three rigid-body
parameters per frame using a robust M-estimator (Tukey bisquare loss
with IRLS), and incorporate a linear drift directly into an augmented
design matrix.  The fitted focal-plane drift slopes are converted to
altitude/azimuth post hoc via the mean WCS Jacobian matrix for
...
Since the off-diagonal terms of $\mathbf{J}_i$ are small
(${\sim}\,5\%$ of the diagonal) and dominated by position angle
rotation, a common $(dx, dy)$ in focal plane is an excellent
approximation.  We therefore adopt the simplest model: the rigid-body
fit operates entirely in focal-plane coordinates,
